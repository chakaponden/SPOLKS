...
void hdl_SIGINT_TCP(int sig, siginfo_t *siginfo, void *context)	// handler for SIGINT (Ctrl+C) withinsignal
{
	...
	if(ind > 1)
	{
	  if(shutdown(listenSock, SHUT_RDWR) < 0)		// deny connection
	    fprintf(stderr, "TCP PID: %d shutdown listenSock signal errno: %d\n", getpid(), errno);
	  if(close(listenSock) < 0)			
		  fprintf(stderr, "TCP PID: %d close listenSock signal errno: %d\n", getpid(), errno);
	  else
		  ind-=2;
	}
	if(ind)   
	{
		if(shutdown(workSock, SHUT_RDWR) < 0)		// deny connection
			fprintf(stderr, "TCP PID: %d shutdown workSock signal errno: %d\n", getpid(), errno);
		if(close(workSock) < 0)		
			fprintf(stderr, "TCP PID: %d close workSock signal errno: %d\n", getpid(), errno);	
		else
			ind--;
	}
...
void hdl_SIGINT_UDP(int sig, siginfo_t *siginfo, void *context)	// handler for SIGINT (Ctrl+C) withinsignal
{
        ...
        if(parentPid == getpid())
	{
	  ...
	  {
		  if(close(workSock) < 0)		
			  fprintf(stderr, "UDP PID: %d close workSock signal errno: %d\n", getpid(), errno);	
		  else
			  ind--;
	  }
	}
...
int startServerTcp(char *hostName, char *port)
{
	...
	*/	
	...
	if(((listenSock) = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP)) < 0)		
   	{
        	fprintf(stderr, "TCP socket errno: %d\n", errno);
        	return -1;
    	}    
	ind += 2;
	...
	/*setsockopt(listenSock, SOL_SOCKET, 
    	...
    	{
        	fprintf(stderr, "TCP bind errno: %d\n", errno);
        	return -1;
    	}
	  ...
	  {	
	    ...
	    */  
		while((retVal = poll(&tempSet, highDescSocket, time_out)) < 0)
		{
		  if(errno != EAGAIN && errno != EWOULDBLOCK && errno != EINTR)
			  {
			    fprintf(stderr, "TCP poll errno: %d\n", errno);	
								// errno == 11 means EAGAIN or EWOULDBLOCK == Try again
								//
			    return -1;    			// errno == 4 means EINTR == Interrupted system call
			  }
		...
		if(retVal)  
		{
		  ...
		  {						// wait for client NON_BLOCKING
			  if(errno != EAGAIN && errno != EWOULDBLOCK && errno != EINTR)
			  {
			    fprintf(stderr, "TCP accept errno: %d\n", errno);	
								// errno == 11 means EAGAIN or EWOULDBLOCK == Try again
		  ...
		  if(workSock >= 0)
		  {
		    ...
		    */
		    ...
		    {
			fprintf(stderr, "TCP recv filePath errno: %d\n", errno);
		    ...
		    */
		    ...
		    {
		      fprintf(stderr, "TCP recv fileSize errno: %d\n", errno);
		      if(errno != EAGAIN && errno != EWOULDBLOCK && errno != EINTR) 
			      return -1;			// errno == 11 means EAGAIN or EWOULDBLOCK == Try again	
								// errno == 4 means EINTR == Interrupted system call
		    }
		    ...
		    {						// that point to EOF (filePointer==fileSize)
		      ...
		      {
			fprintf(stderr, "TCP send fileSize errno: %d\n", errno);
								// errno == 4 means EINTR == Interrupted system call 
			if(errno != EAGAIN && errno != EWOULDBLOCK && errno != EINTR)
				return -1;			// errno == 11 means EAGAIN or EWOULDBLOCK == Try again	
		      } 
...
int serverProcessingTcp(int oldWorkSock, char *oldFilePath, long long oldFileSize)
{
  ...
  */
  ...
  highDescSocket = 1;
  printf("TCP PID: %d file '%s' %lld bytes start downloading\n", getpid(), filePath, fileSize);  
  while(filePointer < fileSize && readBytes)							
  {
    ...
    */
	...
	fprintf(stderr, "TCP PID: %d recv OOB errno: %d\n", getpid(), errno);
								// errno == 4 means EINTR == Interrupted system call 
	if(errno != EAGAIN && errno != EWOULDBLOCK && errno != EINTR)		
	  return -1;						// errno == 11 means EAGAIN or EWOULDBLOCK == Try again
      }
	...
	{		
	  ...
	  */	  
	  ...
	  {							// file exist
	    ...
	    {
	      printf("TCP PID: %d Do you want to redownload %s file? Y/N\n", getpid(), filePath);
	    ...
	    {
	      printf("TCP PID: %d Resume downloading %lld bytes %s file? Y/N\nY = resume, N = redownload\n",
	  ...
	  {
	    fprintf(stderr, "TCP PID: %d send filePointer fileSize errno: %d\n", getpid(), errno);
								// errno == 4 means EINTR == Interrupted system call 
	    if(errno != EAGAIN && errno != EWOULDBLOCK && errno != EINTR)
		    return -1;					// errno == 11 means EAGAIN or EWOULDBLOCK == Try again	
	  } 
	...
	if(errno != EAGAIN && errno != EWOULDBLOCK && errno != EINTR)	
	{
	  fprintf(stderr, "TCP PID: %d recv data errno: %d\n", getpid(), errno);
	  return -1;						// errno == 11 means EAGAIN or EWOULDBLOCK == Try again
	}
      }
  ...
  if(shutdown(workSock, SHUT_RDWR) < 0)				// deny connection
  {
    fprintf(stderr, "TCP PID: %d shutdown workSock errno: %d\n", getpid(), errno);
    return -1;
  }
  if(close((workSock)) < 0)					// close connection
  {
    fprintf(stderr, "TCP PID: %d close workSock errno: %d\n", getpid(), errno);
    return -1;
  }
  ind--;
  return 0;
...
int startServerUdp(char *hostName, char *port)
{
	...
	*/	
	...
	if(((workSock) = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP)) < 0)		
   	{
        	fprintf(stderr, "UDP socket errno: %d\n", errno);
        	return -1;
    	}    
	ind++;
...
int serverProcessingUdp(int oldWorkSock, struct sockaddr_in oldHostAddr, long long oldFileSize, struct sockaddr_in oldClientAddr, int index)
{
  ...
  */
  ...
  tempSet.events = POLLPRI;
  highDescSocket = 1;
  printf("UDP PID: %d file '%s' %lld bytes start downloading port: %d\n", getpid(), filePath, fileSize, clientVect[index].port); 
	...
	{		
	  ...
	  */	  
	  ...
	  {							// file exist
	    ...
	    if(localFileSize == fileSize)
	    {
	      printf("UDP PID: %d Do you want to redownload %s file? Y/N\n", getpid(), filePath);
	      fgets(c, sizeof(c)*sizeof(char), stdin);		// input
	      if(c[0] == 'Y')					// press 'Y'
	      {
		file = fopen(filePath, "wb");			// create file
	    ...
	    {
	      printf("UDP PID: %d Resume downloading %lld bytes %s file? Y/N\nY = resume, N = redownload\n",
								getpid(), (fileSize-localFileSize), filePath);
	      fgets(c, sizeof(c)*sizeof(char), stdin);		// input
	      if(c[0] == 'Y')					// press 'Y'
	      {
    ...
    //puts("server recv data");	
		...
		recvMess = -1;
				    ...
				    (clientAddrRecv.sin_port == clientVect[index].port))	
			{
			    ...
			    {
			      fprintf(stderr, "UDP PID: %d recvfrom data + REMOVE_QUEUE readBytes > recvFlag errno: %d\n", getpid(), errno);
				      return -1;
			    }	
  ...
  //printf("server_close_socket\n");
  if(filePointer == fileSize)
    printf("UDP PID: %d file '%s' %lld bytes successful received\n", getpid(), filePath, fileSize);
  else
  {
    printf("UDP PID: %d some errors with file '%s' %lld bytes\nPID: %d %lld bytes only received\n", 
	   getpid(), filePath, fileSize, getpid(), filePointer);
  }  
  ind--;		    
  return 0;
